...
Delegate::Delegate(QObject *parent)
    : QStyledItemDelegate(parent), m_ptr(new DelegatePrivate) {}

Delegate::~Delegate() { delete m_ptr; }

QSize Delegate::iconSize() const { return m_ptr->iconSize; }

void Delegate::setIconSize(int width, int height) {
  m_ptr->iconSize = QSize(width, height);
}

QMargins Delegate::contentsMargins() const { return m_ptr->margins; }

void Delegate::setContentsMargins(int left, int top, int right, int bottom) {
  m_ptr->margins = QMargins(left, top, right, bottom);
}

int Delegate::horizontalSpacing() const { return m_ptr->spacingHorizontal; }

void Delegate::setHorizontalSpacing(int spacing) {
  m_ptr->spacingHorizontal = spacing;
}

int Delegate::verticalSpacing() const { return m_ptr->spacingVertical; }
...
void Delegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
  ...
  const QRect &contentRect(
      rect.adjusted(m_ptr->margins.left(), m_ptr->margins.top(),
                    -m_ptr->margins.right(), -m_ptr->margins.bottom()));
  ...
  painter->drawLine(lastIndex ? rect.left() : m_ptr->margins.left(), bottomEdge,
                    rect.right(), bottomEdge);
  ...
  messageRect.moveTo(timeStampRect.left(),
...
QRect DelegatePrivate::timestampBox(const QStyleOptionViewItem &option,
  ...
  return QFontMetrics(f)
      .boundingRect(index.data(Qt::UserRole).toString())
      .adjusted(0, 0, 1, 1);
}
